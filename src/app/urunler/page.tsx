                    ...
                    <div className="flex justify-center px-4 lg:px-10">
                        <div className="flex w-full max-w-7xl items-center justify-between whitespace-nowrap border-b border-solid border-secondary/10 py-3 dark:border-white/10">
                            ...
                            <nav className="hidden flex-1 items-center justify-center gap-9 lg:flex">
                                ...
                                <Link href="/urunler" className="text-sm font-medium leading-normal text-primary dark:text-primary">Ürünler</Link>
                ...
                <main className="flex-1">
                    <section className="bg-secondary py-16 text-white">
                        <div className="mx-auto max-w-7xl px-4">
                            <h1 className="text-4xl font-black leading-tight tracking-[-0.033em] md:text-5xl">Ürünlerimiz</h1>
                            <p className="mt-4 text-lg text-white/80">Endüstriyel gaz ürün yelpazemizi keşfedin.</p>
                        </div>
                    </section>
                    ...
                    <section className="bg-background-light py-16 dark:bg-background-dark sm:py-24">
                        <div className="mx-auto max-w-7xl px-4">
                            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
                                {[
                                    { title: "Medikal Gazlar", desc: "Oksijen, Azot Protoksit, Medikal Hava" },
                                    { title: "Endüstriyel Gazlar", desc: "Oksijen, Azot, Argon, Karbondioksit" },
                                    { title: "Kaynak Gazları", desc: "Asetilen, Propan, Karışım Gazlar" },
                                    { title: "Gıda Gazları", desc: "CO2, N2, Karışım Gazlar" },
                                    { title: "Özel Gazlar", desc: "Kalibrasyon, Analitik Gazlar" },
                                    { title: "Kriyojenik Sıvılar", desc: "Sıvı Azot, Sıvı Oksijen, Sıvı Argon" },
                                    { title: "Gaz Ekipmanları", desc: "Regülatörler, Manifoldlar, Vanalar" },
                                    { title: "Güvenlik Ekipmanları", desc: "Dedektörler, Alarm Sistemleri" }
                                ].map((product, i) => (
                                    <div key={i} className="rounded-xl bg-white p-6 shadow-md dark:bg-background-dark">
                                        <h3 className="text-xl font-bold text-secondary dark:text-white">{product.title}</h3>
                                        <p className="mt-2 text-secondary/70 dark:text-white/60">{product.desc}</p>
                                        <button className="mt-4 text-sm font-bold text-primary hover:underline">Detaylı Bilgi →</button>
                                    </div>
                                ))}
                            </div>
                        </div>
